...
## 没有输入摘要的已发布封印是一份基准，不是一次拒绝

v0.23.0 之前发布的每一份 stable 封印都不带输入摘要——那时还没有任何东西记录它们。
读到这样一份封印，原先被当成「答复格式错误」而不是「基准为空」，于是 `release compile`
拒绝。而本域**每一个产品**当前的 stable 恰好都是这样一份封印，所以 v0.23.0 编译不出
任何发布，包括它自己的后继版本。

字段缺席现在按它本来的意思读：关于什么动过一无所知，于是全部投影，每个对象都盖上本次
版本。只有**存在且格式错误**的字段才拒绝。

这处退化直到现在才暴露，因为发布永远跑在**已经发布出去**的那个 Plumb 上，而不是正在被
发布的那个——所以发布引擎里的故障总是迟一个版本才现身。

## 相对的发布根仍然指同一棵树

lane 交给 plumb 的发布根是 `.`，而 Cargo 回答的是绝对 manifest 路径。用后者去剥前者
悄悄失败，座位仍是绝对路径，于是每个 Cargo 对象都「覆盖不到任何被跟踪的叶子」，compile
拒绝。现在根在被剥掉之前先解析成真实路径，两边说的才是同一种形状。
