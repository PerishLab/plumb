...
- Governed lanes are rendered, never written. `plumb lane` derives each one from the release
  ...
  recorded seal: a generated target needs no evidence it cannot already derive. Rendering decides which jobs exist, so a product declaring no binary gets a lane without build, seal, and smoke rather than an empty matrix, which Forgejo never creates and whose dependents block forever. A rendered lane therefore carries no condition render time could have decided; what it cannot decide is what only the run knows — which operating system a matrix job lands on, and which channel the dispatched ref carries — so `runner.os`, the resolved channel, and whether a declared key's input moved are the only conditions a rendered step may hold. The third belongs with the other two because a lane is written once and read by every commit after it, so the hash it turns on is one render time cannot know; an absent answer runs the step, because a condition whose absence skipped one would turn every outage into a guard that passed what it never checked. Drift is noted, never out of
...
- A rendered lane repeats no step it can prove nothing changed under. `[workflow.hash]` declares, per key, the paths whose recorded bytes decide that step's outcome, and the input hash covers those paths, the declared roots themselves, and the forge image, so widening a declaration invalidates its own key. A key names its lane and its own namespace on either side of one slash, because lane names are Plumb's closed enumeration and everything after the slash is the product's to divide. `key://` composes one key's inputs into another's and `suite://` names a classic toolchain inventory Plumb carries, so a step reading what two others read declares that rather than a wildcard, and a suite is one row in Plumb's rules rather than a list every product copies. Cycles and unknown lanes, keys, or suites refuse. The declaration is the product's assertion that these paths decide that outcome; Plumb only holds it against what Git recorded. The lock recording whether a key already ran sits beside the release objects and never in the repository, because only a run that passed may record one and a run may not write to the tree it is judging. Every failure on that path runs the step: an unreadable rig, an unreadable lock, an undeclared key, and a forced run all say run, and a lock that cannot be written warns without failing the step that earned it. A lost lock is a repeated step, which is the answer this mechanism must always be able to fall back to. A declaration is only as good as what it left out, and the trap is a step that runs this tool against this repository: `cargo test` here asserts `plumb doctor` finds the tree true, so its input is the whole tree and not the crates alone, while the formatter, the linter, and the release check read only what Cargo reads. Nothing mechanical catches a declaration that is too narrow, because only the product knows what its own steps read; what a narrow one buys is a step skipped that would have failed. Each key renders as its own named step and one step asks for all of them at once, so a lane makes one round of enquiry rather than one per proof and a failure lands on the step that owns it, which is the only diagnosis a forge that cannot serve job logs still offers. A key spanning several steps holds them under one answer and records the lock in the last of them, because skipping a package install while running the type check needing its modules fails for a reason no declaration described. A key no product declared renders as a step without a condition. The lock authority reaches the lane as job environment, because every step that proves something also records it, so narrowing the seat to some steps would name every step that matters. A token scoped to the lock seat alone is what keeps that ordinary: the guard writes one small object per key and can reach nothing a release owns. One seat holds every repository's locks, because a lock is a run's own bookkeeping and not a product's output, and sixteen buckets for it would cost more than the repetition it saves. The repository names its own room inside that seat, or two products sharing a key name would push each other out and neither would ever hold. The room comes from the run, so a lock cannot be read or written anywhere else: what would otherwise be a rule about who may record one is instead a thing only a run can do. Asking and recording both tolerate their own failure, because the released tool a lane installs is by construction older than the lane that installs it and a step must not fail for having been told to record something the tool it was given cannot spell. A repository whose product is the tool installs the released tool beside its own build, because asking whether a step may be skipped must not first compile the thing that would have run it.
...
- Those numbers travel without a release. Plumb's rules and its lane templates publish as immutable objects to Plumb's own depot, and a synced seat answers before the compiled bytes do, while the compiled bytes remain permanently, because a tool that cannot run without reaching the network fails exactly where it is needed most. The guarantee the rules directory carries survives the move: the depot's authority is Plumb's, written by Plumb's own lane from Plumb's own recorded roots, so a product still cannot grant itself a shape by declaring one, it can only follow the shape Plumb published. A version is a timestamp and nothing else, because a total order needs no verb to advance and no judgement to compare, and the channel pointer naming the current one is the only movable object in the store. Nothing pins, so every repository follows the channel and what a run must record for a release to be reproducible is the version it held. Reading is local: the held seat answers and the network is never asked, so a stale seat is an observation while a seat that exists and cannot be read is blind, which is the one thing a floor may not silently paper over. Two laws stand on it, that the roots a repository records are carried by the held version under the same digest, and that the running binary is at or above the floor that version declares. Publishing refuses roots carrying uncommitted change, because a store of record may only carry what Git already holds.
- A version's release notes publish to that same depot under the version itself, with a manifest recording their digests and the commit the tree stood at. Notes are staged in the repository's temporary seat and cleared once the depot holds them, so they never enter the tree and never come before a reviewer: a note describes a version that is already a distributed fact, and review governs proposals rather than facts.
- Every declared attachment is an object the seal can speak about, images included: an image is its `Containerfile` and, where it wraps this release's archive, the release itself — because such an image moves with the release whatever its source did, and an input hash that said otherwise would be a claim rather than evidence. A ship object is one attachment, not one package: every package an attachment declares carries one input hash, one version, and ships together, because a unit the language forced into several boxes is still one unit. A seal records that hash per object — the path, mode, and recorded object of every tracked leaf under the derived roots, plus the pinned toolchain, stamped with the version those inputs first appeared in — and `[release.depends]` adds only the edges no convention derives. It measures what Git recorded, never the bytes lying in the working seat, so a dirty runner cannot move the answer.
- A projection never skips. The input hash says what moved since the last stable; it does not decide what ships. A rehearsal is the one thing an attachment cannot always do for every package it holds: a package requiring a sibling of the same attachment cannot be built from its packaged form until that sibling is published, so the rehearsal says so and the ordered publish is what proves it. Every declared object projects at the release version, because a published identity carries that version and not shipping one punches a hole in the sequence that no later release can fill.
- A brief is what a reader trusts first, so `doctor` names every installed brief whose version is not the running binary's, wherever it runs. It is an observation and not a finding: the operator's seats are not the repository's business, and a repository that is true stays true beside a stale brief.
