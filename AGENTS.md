...
- A branch is a line and a tag is a point. `release/vX.Y.Z` accumulates the
  ...
  removable handle.
- Every release point Plumb releases from is stamped by Plumb. `plumb stable
  freeze` stamps the stable one on its way through, and `plumb stable stamp`
  stamps an exact one on the line it belongs to, so an exact release starts from
  a point a mechanism made rather than a hand. Both refuse to move a point that
  already stands, and each refuses the other's channel. `plumb stable retract`
  removes a point. Retraction reads the release authority first and continues only on a
