...
- A branch is a line and a tag is a point. `release/vX.Y.Z` accumulates the
  ...
  removable handle.
- A line and its points are different objects and different segments. `plumb
  stable` opens, picks onto, freezes and rejoins the line and touches no point;
  `plumb release stamp` fixes a version name to the head of the line it names,
  and `plumb release retract` removes one, both for either channel, because a
  point is what a tag means and that is the segment a tag begins. So an exact
  release starts from a point a mechanism made rather than a hand, and a stable
  release is declared before it runs: dispatching stable refuses unless its
  point already stands at the line head. Retraction reads the release authority first and continues only on a
