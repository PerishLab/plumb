...
impl Fixture<'_> {
    pub fn command(&self) -> Command {
        self.binary(Path::new(env!("CARGO_BIN_EXE_plumb")))
    }

    pub fn controller(&self) -> Command {
        self.binary(crate::marker::controller())
    }

    fn binary(&self, binary: &Path) -> Command {
        let mut held = Command::new(binary);
        let path = format!(
            "{}:{}",
            self.tools.display(),
    ...
    pub fn seed(&self) {
        ...
        super::support::stock(
            ...
            &[("rules/workflow.toml", &crate::marker::workflow("probe"))],
        );
...
case "$operation" in
  put-object)
    if [ -n "${FAKE_DEPOT_RACE:-}" ] && [ "${key##*/}" = latest.json ]; then
      cp "$FAKE_DEPOT_RACE" "$path"
    fi
    ...
    cp "$body" "$path"
    if [ "${FAKE_DEPOT_LOST:-}" = true ] && [ "${key##*/}" = latest.json ]; then
      echo "Connection broken after write" >&2
      exit 1
    fi
    ;;
  head-object)
    if [ "${FAKE_DEPOT_UNKNOWN:-}" = true ] && [ "${key##*/}" = latest.json ]; then
      echo "Connection broken" >&2
      exit 1
    fi
    [ -f "$path" ] || {
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --retry|--retry-delay|--retry-max-time|--max-time) shift 2 ;;
...
case "$url" in
  */ship.json)
    if [ -n "${FAKE_COMPLETION_STATUS:-}" ]; then
      printf '%s' "$FAKE_COMPLETION_STATUS"
      exit 0
    fi
    ;;
  */inventory.json)
...
case "$url" in
  ...
  http://127.0.0.1:*/channels/*) route=${url#http://}; path="$FAKE_S3_ROOT/depot/${route#*/}" ;;
  *) exit 1 ;;
esac
if [ -f "$path" ]; then
