mod identity;
#[allow(dead_code)]
#[path = "../../../../src/command/ship/adaptor/projection.rs"]
mod projection;
...
fn direct() {
    ...
    for (name, body) in [("pnpm", PNPM), ("curl", curl.as_str())] {
...
fn publish(
    ...
    let content = plumb::depot::sha(&std::fs::read(root.join("workload.tgz")).unwrap());
    let request = serde_json::json!({
        ...
        "reuse":{"type":"workload","source":format!("https://inventory.invalid/v2/blobs/sha256/{content}")},
    });
