...
fn plain() {
    ...
    assert!(text.contains("name: ship"), "{text}");
    assert!(!text.contains("guard_contexts"), "{text}");
...
fn bootstrap() {
    ...
    let text = root.rendered();
    assert!(text.contains("name: ship"), "{text}");
    assert!(!text.contains("release evidence"), "{text}");
}
...
fn local() {
    ...
    let text = root.rendered();
    assert!(text.contains("name: ship"), "{text}");
    assert!(!text.contains("guard.atom.yml"), "{text}");
}
