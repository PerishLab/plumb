...
fn catalog() {
    ...
    assert_eq!(all["rules"].as_array().map(Vec::len), Some(113));
    assert_eq!(mechanized["rules"].as_array().map(Vec::len), Some(76));
    assert_eq!(prose["rules"].as_array().map(Vec::len), Some(37));
