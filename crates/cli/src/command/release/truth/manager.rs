...
mod tests {
    ...
    fn spec(root: &std::path::Path) -> Spec {
        ...
        .unwrap();
        Spec::read(&path).unwrap()
    }

    #[test]
    fn stable_pins_its_version_and_adds_a_canonical_pair() {
        let root = tempfile::tempdir().unwrap();
        let out = root.path().join("out");
        write(&spec(root.path()), "stable", "v1.2.0", &out).unwrap();
        let pinned = std::fs::read_to_string(out.join("manage.sh")).unwrap();
        let canonical = std::fs::read_to_string(out.join("canonical/manage.sh")).unwrap();
        assert!(pinned.contains("VERSION=${DEMO_VERSION:-v1.2.0}"));
        assert!(canonical.contains("VERSION=${DEMO_VERSION:-}"));
        assert!(canonical.contains("CHANNEL=${DEMO_CHANNEL:-stable}"));
        assert!(out.join("manage.ps1").is_file());
        assert!(out.join("canonical/manage.ps1").is_file());
    }

    #[test]
    fn a_prerelease_carries_only_its_pinned_pair() {
        let root = tempfile::tempdir().unwrap();
        let out = root.path().join("out");
        write(&spec(root.path()), "rc", "v1.2.0-rc.1", &out).unwrap();
        let pinned = std::fs::read_to_string(out.join("manage.sh")).unwrap();
        assert!(pinned.contains("CHANNEL=${DEMO_CHANNEL:-rc}"));
        assert!(pinned.contains("VERSION=${DEMO_VERSION:-v1.2.0-rc.1}"));
        assert!(!out.join("canonical").exists());
        assert!(write(&spec(root.path()), "rc", "v1.2.0-rc.1", &out).is_err());
    }
}
