...
impl Seat<'_> {
    ...
    fn baseline(&self) -> Result<BTreeMap<String, Input>, String> {
        ...
        let Some(pointer) = crate::command::release::verify::optional(&url)? else {
            return Ok(BTreeMap::new());
        };
        let seat = pointer
            .pointer("/seal/url")
        ...
        let Some(seal) = crate::command::release::verify::optional(seat)? else {
            return Ok(BTreeMap::new());
        };
    ...
    fn crates(&self, held: &mut BTreeMap<String, Vec<PathBuf>>) -> Result<(), String> {
        ...
        let seats =
            crate::command::release::workspace::Workspace::read(&self.0.root)?.seats(&self.0.root);
