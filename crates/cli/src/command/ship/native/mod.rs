...
pub(super) fn run(request: Request<'_>) -> Result<PathBuf, String> {
    request.run()
}
...
impl Request<'_> {
    fn run(self) -> Result<PathBuf, String> {
        ...
        let original = self.content(temporary.path())?;
    ...
    fn content(&self, temporary: &Path) -> Result<PathBuf, String> {
        let path = temporary.join(self.archive);
        match self.build.reuse.kind.as_str() {
            "none" if self.build.reuse.source.is_empty() => {
                crate::command::ship::package::product(self.spec).build(
                    crate::command::ship::package::Build {
                        target: self.target,
                        version: &release::channel::base(&self.release.version)?,
                        channel: "stable",
                        commit: required("PLUMB_RELEASE_COMMIT", &self.release.commit)?,
                        artifacts: temporary,
                    },
                )?;
